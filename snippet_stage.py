...
def _run_full_shell_compile_gate(*, cfg, task_dir, module_info, combine_info, method_name, snippet_code):
    # Known compatibility gap: Yosys 0.64 cannot parse some unpack lines like
    # `in_dis2iss.req[i][j].field = ...` generated by current combine mapping.
    # This is unrelated to method snippet quality, so skip this gate for ISU
    # until mapping/codegen is made Yosys-friendly.
    if module_info.get("module_type") == "Isu":
        return True, "SKIPPED_FULL_SHELL_COMPILE_GATE: unsupported Yosys syntax for current ISU mapping"
